...
from patient_embedding.shared.io import ensure_dir, read_text, write_text
...
def run_analysis(rnd: random.Random, label: str, cos_func: Callable[[str, str], float], out_dir: Path):
    ...
    diff_array = df_pairs["cosine_diff"] # difference between cosine score and judge score
    
    plots_dir = out_dir / "plots"
    os.makedirs(plots_dir, exist_ok=True)
    handle_plotting(label, plots_dir, judge_array, cos_array, diff_array)
    write_discordant(
                    out_dir, 
                    df_pairs, 
