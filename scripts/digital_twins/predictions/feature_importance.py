...
from scripts.digital_twins.predictions.classical_ml import make_classifier, CLASSIFIER_FAMILY
from scripts.shared.utils import VectorSource, VitalsStrategy, ClassifierFamily
...
def load_best_params(model_name: str, source: VectorSource, vitals_strategy: Optional[VitalsStrategy]=None) -> dict:
    ...
    Args:
        ...
        vitals_strategy (Optional[VitalsStrategy]): When set with source=FEATURE, splices a strategy suffix into the JSON filename to locate the per-strategy grid-search result. Ignored for EMBEDDED.
    ...
    results_json_path = Path(os.environ['RESULTS_DIR']) / f"grid_search_ml_results_{source.name}{strategy_suffix}.json"
...
def refit_best_model(
    ...
    y_train: np.ndarray,
    vitals_strategy: Optional[VitalsStrategy]=None,
    classifier_family: Optional[ClassifierFamily]=None,
) -> Pipeline:
    ...
    Args:
        ...
        vitals_strategy (Optional[VitalsStrategy]): Forwarded to model_cache_path / load_best_params / make_classifier so the cache lookup and refit-fallback target the right per-strategy artifact. Ignored for EMBEDDED.
        classifier_family (Optional[ClassifierFamily]): Forwarded to make_classifier so the refit-fallback builds the right numeric branch shape under INDICATOR / ASYMMETRIC.

    Returns:
        Pipeline: fitted sklearn Pipeline
    """
    cache_path = model_cache_path(model_name, source, vitals_strategy)
    ...
    base_estimator_pipeline = make_classifier(models[model_name], vitals_strategy, classifier_family)
    hyperparams = load_best_params(model_name, source, vitals_strategy)
...
def plot_feature_importance(
    ...
    Args:
        ...
        vitals_strategy (Optional[VitalsStrategy], optional): When set, splices a strategy suffix into the PNG filename to prevent collisions across the three FEATURE sensitivity arms. Defaults to None.
    """
    ...
    ax.set_title(title)
    fig.tight_layout()
    strategy_suffix = f"_{vitals_strategy.name}" if vitals_strategy is not None else ""
    save_path = Path(os.environ['RESULTS_DIR']) / "feature_importance" /\
        f"feature_importance_{model_name}{strategy_suffix}.png"
...
def plot_pca_k_vs_roc(
  ...
  versus K
    ...
    base_models = {
        ...
        "gradient_boosting": HistGradientBoostingClassifier(random_state=int(os.environ['SEED'])),
...
def write_feature_importance_summary(
    ...
    Args:
        ...
        vitals_strategy (Optional[VitalsStrategy], optional): When set, splices a strategy suffix into the summary JSON filename so the three FEATURE sensitivity arms write to disjoint paths. Defaults to None.
    """
    strategy_suffix = f"_{vitals_strategy.name}" if vitals_strategy is not None else ""
    save_path = Path(os.environ['RESULTS_DIR']) / "feature_importance" / f"feature_importance_summary{strategy_suffix}.json"
    os.makedirs(save_path.parent, exist_ok=True)
...
def main():
    ...
    # FEATURE pass: per-strategy loop, per-family load (matrices diverge under ASYMMETRIC)
    source = VectorSource.FEATURE
    for strategy in VitalsStrategy:
        print(f"Feature importance pass: {source.name} running (vitals strategy: {strategy.name})...", flush=True)
        # Per-family matrix dict: under ASYMMETRIC the two families diverge; otherwise both keys point to the same load
        matrices_by_family: dict[ClassifierFamily, tuple] = {}
        if strategy == VitalsStrategy.ASYMMETRIC:
            for family in (ClassifierFamily.LINEAR, ClassifierFamily.TREE):
                (X_train_f, y_train_f) = load_data_set(train_ids, source=source, vitals_strategy=strategy, classifier_family=family)
                (X_test_f, y_test_f) = load_data_set(test_ids, source=source, vitals_strategy=strategy, classifier_family=family)
                matrices_by_family[family] = (X_train_f, y_train_f, X_test_f, y_test_f)
        else:
            (X_train_s, y_train_s) = load_data_set(train_ids, source=source, vitals_strategy=strategy)
            (X_test_s, y_test_s) = load_data_set(test_ids, source=source, vitals_strategy=strategy)
            shared = (X_train_s, y_train_s, X_test_s, y_test_s)
            matrices_by_family[ClassifierFamily.LINEAR] = shared
            matrices_by_family[ClassifierFamily.TREE] = shared

        summary: dict[str, list[dict]] = {}
        for model_name in MODEL_NAMES:
            family = CLASSIFIER_FAMILY[model_name]
            (X_train, y_train, X_test, y_test) = matrices_by_family[family]
            print(f"Running {model_name} feature importance under {source.name} vectors ({strategy.name})...")
            model_pipeline = refit_best_model(model_name, source, X_train, y_train, vitals_strategy=strategy, classifier_family=family)
            ...
            plot_feature_importance(importances, feature_names, model_name, direction_signs=direction_signs, vitals_strategy=strategy)
        ...
        write_feature_importance_summary(summary, vitals_strategy=strategy)
