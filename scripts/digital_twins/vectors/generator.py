...
from scripts.data_loading.feature_vector import (
...
RECORD_EVERY = 1000
FEATURE_DATAFRAME_PATH = Path(os.environ['FEATURE_DATAFRAME_PATH'])

def _process_patient(sliced_json: Dict) -> Tuple[str, pd.Series]:
    """Return patient's id paired with their feature vector
    ...
    """
    return (sliced_json['patient_id'], generate_feature_vector(sliced_json))

def _sanity_check_report(df: pd.DataFrame):
    """Sample and perform a sanity check on a random sample of patient feature vectors

    Args:
        df (pd.DataFrame): All feature vectors
    """
    n_samples = 10
    ...
    with open(CATEGORICAL_LEVELS_PATH, 'r') as f:
        ...
        def get_patient_dump(patient_id) -> str:
            narrative_path = Path(os.environ['NARRATIVES_DIR']) / f"{patient_id}.md"
    ...
    with multiprocessing.Pool(processes=int(os.environ['NUM_WORKERS_NON_LLM_TASK'])) as thread_pool:
        for (i, (patient_id, series)) in enumerate(thread_pool.imap_unordered(_process_patient, load_patient_data())):
            ...
            if (i + 1) % RECORD_EVERY == 0:
                print(f"Created {i+1} feature vectors...", flush=True)

    print(f"Created all feature vectors ({len(results)} patients passed the history-window filter).", flush=True)
    # Index is patient ID
    ids, vectors = zip(*results)
    vectors_df = pd.DataFrame(vectors, index=ids)
    vectors_df.index.name = "patient_id"
    os.makedirs(FEATURE_DATAFRAME_PATH.parent, exist_ok=True)
    vectors_df.to_parquet(FEATURE_DATAFRAME_PATH)
    print(f"Saved DataFrame shape {vectors_df.shape} to {FEATURE_DATAFRAME_PATH}", flush=True)
