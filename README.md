...
* **Causal Random Forest — Treatment-Effect Supplement to TRD Prediction** — An optional causal companion to the prediction pipeline, living in its own notebook (`notebooks/causal_random_forest.ipynb`). Where the classification pipeline asks *who will become TRD?*, this asks the causal question over the **same TRD outcome**: *which treatment behaviors causally change a patient's probability of becoming TRD, and for whom?* It estimates conditional average treatment effects (CATE) via `CausalForestDML` for each candidate treatment that clears an overlap/positivity check — a prevalence band of [5%, 95%]. On the current 9,724-patient cohort, augmentation (~0.9% prevalence) fails that band and is dropped; polypharmacy (thresholded; `>= 2` ≈ 63%) and adequate-trial-≥1 (~6.4%, thin — ~623 treated, so expect noisy CATE) survive. Evaluation surface: Qini coefficient, uplift curves, doubly-robust CATE calibration, CATE distribution plots, and subgroup ATE tables stratified by the top SHAP-ranked moderators. CATE metrics are incompatible with the ROC/PR/Calibration harness of the main pipeline, which is why this lives in its own notebook. **Implementation status:** data loading (via the shared `create_train_test_split`, `load_feature_matrix`, and `load_trd_set` helpers), the `get_treatment_indicator` treatment dispatcher, one-hot encoding of the eight categorical moderators, and the per-treatment `CausalForestDML` fit loop are implemented; test-set CATE estimates for the two surviving treatments are persisted to `notebooks/figures/cate_test.parquet` (keyed by `patient_id`; polypharmacy ATE ≈ +0.023, adequate-trial ATE ≈ +0.037, both small positive effects on TRD probability — but see the calibration caveat below before reading those as causal). Per-treatment CATE distribution histograms are also built and saved to `notebooks/figures/`. **This supplement is currently a proof-of-concept for the evaluation harness, not a publishable result on the present cohort.** Held-out doubly-robust CATE *calibration* shows neither surviving treatment carries usable heterogeneity: polypharmacy's calibration R² is small and positive (well-estimated, but little true heterogeneity to find — per-patient CATE barely beats the single ATE) and adequate-trial's is negative (the CATE ranking is anti-informative, an expected symptom of its thin ~6.4% treated overlap starving the estimator). The exact R² is **run-dependent** — it is built from freshly cross-fit `DRTester` nuisances (a RandomForest outcome regressor + propensity classifier under `cv=5`) and computed over only ~10 bins, so it carries real sampling noise; pin the nuisance seeding before quoting a number. The positive ATEs above are therefore best read as residual confounding by indication (sicker patients receive these treatments *and* progress to TRD), not treatment harm. The value here is a reusable, treatment-agnostic verdict surface for the forthcoming data overhaul and alternative treatment definitions. The `DRTester` nuisance fit, the doubly-robust calibration R², the per-treatment calibration scatter plots (`CalibrationEvaluationResults.plot_cal`, saved to `notebooks/figures/{treatment}_calibration.png`), the formal BLP heterogeneity test (`evaluate_blp` — both treatments underpowered: polypharmacy slope ≈ 1.18 but p ≈ 0.06, adequate-trial slope ≈ −1.07 and non-significant, consistent with the calibration verdict) plus its hand-rolled BLP scatter (`notebooks/figures/BLP_scatter_{treatment}.png` — econml exposes no plot method for `BLPEvaluationResults`, so the DR pseudo-outcome is scattered against predicted CATE with a `np.polyfit` line whose slope equals `blp_est` as a positional-alignment check), and the per-test-patient α similarity-weight audit (`audit_alpha_neighbors` — reconstructs the GRF similarity kernel over training patients from the inner `econml.grf.CausalForest`'s leaf-assignment `apply`, surfacing which training patients drive a given patient's CATE), the uplift/Qini and TOC curves (`evaluate_uplift` with `metric='qini'`/`'toc'`, plotted via `UpliftEvaluationResults.plot_uplift` and saved to `notebooks/figures/{qini,toc}_{treatment}.png`; both treatments null — polypharmacy Qini ≈ +0.007, adequate-trial ≈ −0.019, with threshold-hugging p-values that must not be quoted as findings), and the SHAP moderator ranking (`CausalForestDML.shap_values` → mean|SHAP| over test patients; the top moderators for both treatments are data-density/utilization proxies — `pre_anchor_history_days`, encounter/inpatient/emergency counts — not clinical phenotype, echoing the chronology-confounding concern the main pipeline's sanity checks police), and the SHAP-moderator subgroup ATE tables (quartile-bin `cate_test` by each moderator via `pd.qcut`, per-bin mean CATE written to treatment-tagged CSVs and bar plots at `notebooks/figures/subgroup_ATE_{treatment}_{feature}.{csv,png}`) are now built — completing the notebook's evaluation surface. The subgroup tables sharpen the same verdict: the only structured cross-quartile gradient is a monotone rise in estimated effect with *utilization* (`in_patient_days`, `num_encounters`) for both treatments — the data-density confound made concrete, not a clinical moderator — while age and emergency counts are flat and no subgroup flips sign, so the cut surfaces no clinically actionable heterogeneity. This quartile-bin view is now supplemented — not replaced — by direct CATE-vs-feature association plots that add the full 1,945-patient resolution the four quartile bars discard, with the quartile histogram and its `subgroup_ATE_*` tables shipping alongside. The numeric-moderator version (per-patient CATE scattered against the raw feature value with a Spearman ρ, a `np.polyfit` trend line, and a formatted ρ/p title) has landed in both the notebook and the pipeline's `evaluate_subgroup_ate` helper, and it sharpens the same confound: the utilization moderators carry the association (`num_encounters` ρ ≈ 0.32, `in_patient_days` ρ ≈ 0.17, both p < 1e-14) while the SHAP-dominant `pre_anchor_history_days` shows a weak, non-significant raw ρ ≈ 0.03 — its forest importance lives in nonmonotone/interaction structure a rank correlation cannot see, which is itself the argument for shipping both views. The categorical version (per-category CATE box plot with a Kruskal–Wallis / eta-squared effect size) is deferred and low-priority since the top moderators are all continuous — tracked in `writeup/TODO.txt`. **Generalization (in progress):** the notebook's eval surface is complete, and its treatment-agnostic kernel is being refactored into a `scripts/pipeline/causal/` package plus a SLURM array job that sweeps a curated set of *actionable* treatments (prescribing/behavior only, nonzero-vs-zero binarized), auto-screens each candidate against the tunable overlap band (currently [10%, 90%]), and ranks survivors on a leaderboard by out-of-sample calibration R² and multiple-testing-corrected BLP significance — a hypothesis-generating scan for any treatment with usable heterogeneity, not a confirmatory result. **Status (build + validation code-complete; re-derivation RUNNING on the current cohort — 2026-07-20):** the methodological review (`writeup/CRF_AnalysesConsiderations_07.08.2026.docx`) has been **adopted** for the causal work (Paper 1's prediction pipeline is untouched), superseding the original 11-candidate historical binary-"ever" sweep. The treatment is now the antidepressant **class started at the first-AD (index) anchor**, run as active-comparator **pairwise** contrasts over the three dense pairs of the SSRI/SNRI/bupropion index classes (reference/baseline arm = the more common class in each pair; contrasts are keyed comparison-arm-first per the mentor convention that the first-named arm is the treatment T=1, so a positive CATE means the first-named arm raises P(TRD)); the medication-burden variables (polypharmacy, benzodiazepines, hypnotics, NSAIDs) and prior-history trial features stay in the covariate set, and `adequate_trial`/`augmentation` are split off into their own landmark / second-stage analyses rather than serving as baseline treatments. Refactor state: `treatment_registry.py` holds the three dense contrasts, keyed comparison-arm-first (`snri_vs_ssri`, `bupropion_vs_ssri`, `bupropion_vs_snri` — first-named arm = comparison = T=1, baseline = the more common arm; the seven pairs touching mirtazapine/vortioxetine were culled 2026-07-15 on mentor guidance, and those two names are no longer imported), each a `{key, display_name, reference_arm, comparison_arm}` spec whose arms are `med_definitions` class constants. In `core.py`, `get_AD_mappings` (each patient's index-anchor AD class, derived from the anchor-prescription file exactly as `load_patient_data` selects the anchor) and the reworked `build_treatment` (returns a keep-mask plus a 0/1 flag — 1 = comparison arm, 0 = reference arm, all other arms and unmapped patients excluded, replacing the old uniform-`>0` binarization) are built and verified against real data. `load_encoded_data`, the overlap gate, the shared `fit_dr_tester`, and the calibration/BLP/uplift/SHAP-moderator/subgroup-ATE eval helpers carry over treatment-agnostic; the `DRTester` nuisance fit is still built once by `fit_dr_tester` and shared across all eval helpers rather than refit per eval, since the `cv=5` cross-fit is the expensive step. `core.py` and `run_one.py` are now complete: `fit_causal_forest` (restricts to the contrast's two arms via the keep-mask, checks overlap **within** the pair, returns `None` on a skip) and the `fit_and_evaluate` orchestrator are written and verified, and `run_one.py` is the Slurm-array entry point — it reads `SLURM_ARRAY_TASK_ID`, indexes `TREATMENT_REGISTRY`, loads the encoded split once, stamps each contrast's arm counts (including the minority-arm N, recomputed via `build_treatment` so it survives even an overlap skip), calls `fit_and_evaluate`, and writes one per-contrast metrics JSON (`results_{key}.json` under `ARTIFACTS_DIR/causal_pipeline`) — the returned eval dict merged with the arm-count fields on a pass, or a `passed_overlap=False` stub carrying just the arm counts on a skip. `build_leaderboard.py` is now complete too: it globs the `results_{key}.json` files, partitions passed contrasts from overlap skips, Benjamini-Hochberg-corrects (`statsmodels multipletests`, `fdr_bh`) across the passed contrasts' BLP p-values, flattens every record (passers and skips) into a uniform row via `dict.get` defaults so a skip fills NaN rather than raising, marks `is_hit = cal_r_squared > 0 AND blp_significant`, and writes a single `leaderboard.csv` sorted hits-first then by corrected BLP p-value (skips sink to the bottom). The SLURM wiring is in place as well, mirroring the prediction pipeline's orchestrator pattern. `slurm_jobs/pipeline/causal/causal_forest_orchestrator.sbatch` is the entry point: it `--parsable`-submits `causal_forest_sweep.sbatch` (`--array=0-2`, one contrast per task, CPU-only — no `--gres=gpu` — on partition `c3` under the `causal_forest_env` prefix, right-sized to 24 cores / 64 GB since the job holds no large in-memory structure), captures the array job id, chains `causal_leaderboard.sbatch` with `--dependency=afterok:<arrayjob>` so `build_leaderboard` runs exactly once and only after all three contrasts succeed, then chains `causal_validate.sbatch` `afterok` the leaderboard so the sign-flip/transitivity validation suite runs strictly last (it also writes a `cancel_causal.sh` teardown list). The single `results/causal_pipeline` mirror is the rsync at the tail of `causal_validate.sbatch` (removed from the leaderboard job), so it captures `leaderboard.csv` and `validation_report.json` too. Submit the orchestrator **from the repo root**, not the sweep/leaderboard/validate directly, or the chain never fires. The pipeline was code-revised 2026-07-17: contrast keys flipped to comparison-arm-first (mentor naming convention, first-named = treatment T=1); each `results_{key}.json` now records `ate_res` (ATE + 95% CI via `CausalForestDML.ate`/`ate_interval`) and a CATE-vs-DR Spearman (`dr_spearman_rho`/`dr_spearman_pval`) alongside the linear BLP slope; and a standalone `scripts/pipeline/causal/validate.py` sanity suite (sign-flip antisymmetry + a descriptive, non-doubly-robust transitivity check) is complete and chained last by the orchestrator, decoupled from the sweep/registry/leaderboard so it never contaminates the BH correction. The causal orchestrator is now **running (2026-07-20) to derive the numbers** on the new cohort; the stale old-key `leaderboard.csv`/results/figures were moved aside to a backup dir so the fresh run lands clean, and every prior causal number stays void until this run completes. Design resolved (2026-07-14; culled to three pairs 2026-07-15): pairwise (not multi-arm), the three dense pairs (SSRI-SNRI, SSRI-bupropion, SNRI-bupropion) in the registry with the overlap floor at 10% — all three clear overlap comfortably on the accepted cohort; the mirtazapine/vortioxetine pairs were dropped rather than carried as exploratory survivors. Still with the mentor: whether the landmark/second-stage (Trial 2/3) analyses belong in Paper 1 at all. **Inclusion in the manuscript is pending mentor decision — it may or may not be needed.** Its dedicated `causal_forest_env` (econml + scikit-learn 1.6.1, isolated from the main env because econml caps scikit-learn below 1.7) is already provisioned by `setup_envs.sh` — see Environment Setup.
