...
* **FEATURE-side vitals-handling sensitivity analysis** *(planned)* — Three-arm sensitivity comparison over how the FEATURE classical-ML pipeline handles the three cohort-averaged vital columns (`bmi`, `bp_sys`, `bp_dias`), which sit at 63-69% missingness across the cohort with an 8-10 percentage-point TRD-stratified missingness delta (diagnostic in `notebooks/cohort_investigation.ipynb`). The investigation surfaced that median imputation was squashing the engagement-disparity signal onto the cohort median and producing a literature-incoherent protective-BMI red bar in the LR feature-importance plot. Rather than commit to a single handling strategy on first-principles grounds, the FEATURE pipeline iterates three arms — primary arm reported, other two reported as sensitivity analyses. **Arms.** *DROP* strips the three vital columns at `load_data_set` time; all four classifiers see the same vital-stripped matrix; SimpleImputer is a no-op on the resulting numeric block. *INDICATOR* retains the vitals; LR uses `SimpleImputer(strategy='median', add_indicator=True)` so three `missingindicator_*` boolean columns are appended to the LR matrix; tree models drop the imputer entirely and rely on NaN-native splits. *ASYMMETRIC* strips vitals from the LR matrix (matching DROP's LR matrix) while keeping vitals with NaN-native handling for trees (matching INDICATOR's tree matrix). **Architecture.** A new `VitalsStrategy` enum (`DROP`, `INDICATOR`, `ASYMMETRIC`) lives in `scripts/shared/utils.py` alongside `VectorSource`. `classical_ml.py::main()` iterates strategies inside the FEATURE branch of its existing VectorSource loop: 12 FEATURE fits per run (4 classifiers × 3 strategies) plus 4 EMBEDDED fits. Artifact filenames extend the existing `{model}_{source}` suffix to `{model}_{source}_{strategy}` when strategy is set; EMBEDDED artifacts retain the shorter suffix. EMBEDDED source is single-pass — `VitalsStrategy` does not apply (no NaN, no named vital columns; existing embeddings remain valid; the strategy loop is gated to skip when `source == EMBEDDED`). Not an `.env` addition — `VitalsStrategy` is a pipeline-internal iteration axis. **Global classifier change.** `GradientBoostingClassifier` is swapped for `HistGradientBoostingClassifier` across both sources and all arms so the sensitivity comparison stays apples-to-apples; classic GB does not handle NaN and would force per-arm classifier identity, polluting the comparison. The `HYPERPARAMETERS["gradient_boosting"]` grid is rewritten for HistGB's API (`learning_rate` / `max_iter` / `max_leaf_nodes` / `l2_regularization`). **Comparison surface.** After the FEATURE strategy loop completes, an aggregator reads the three `classical_ml_results_FEATURE_*.json` files and emits `vitals_sensitivity.csv` to `RESULTS_DIR/` (12 rows × 10 columns: `strategy`, `classifier`, plus the eight metric keys) and six grouped-bar `vitals_delta_*.png` plots (one per metric: `roc_score`, `auprc`, `brier_score`, `weighted_calibration_error`, `calibration_slope`, `calibration_intercept`), mirroring the `ablation_delta_*` plot pattern; the `roc_score` PNG carries asymmetric CI whiskers, the other five carry bars only. Lives inside `classical_ml.py`'s main loop rather than as a separate orchestrator because each arm is a different feature-matrix shape with the same classifier identity, same hyperparameter grid, same train/test split, and the same metric harness — no narrative regeneration or embedding regeneration needed.
