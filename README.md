...
* **`trd_prediction_computation.py`**:
  ...
  * **Analysis & Metrics**:
    * **Discrimination**: ROC AUC (with bootstrapped 95% CI bands; numeric CI bounds emitted as `roc_score_ci_low` / `roc_score_ci_high` alongside the point estimate in `results.txt` and `summary.csv`), AUPRC.
...
* **`classical_ml.py`**:
  ...
  * **Metrics Reporting**: For each `(classifier, VectorSource)` pair, the shared `compute_metrics` from `trd_prediction_computation.py` is invoked on the held-out test labels and `predict_proba` output. A formatted text block (uppercase `{MODEL}_{SOURCE} Metrics:` header, four-space indented metric lines) is appended to `results.txt` in `RESULTS_DIR`, mirroring the KNN pipeline's block format so both classical-ML and neighbor-weighted exact numbers live in a single file. Eight keys per block: `roc_score`, `roc_score_ci_low`, `roc_score_ci_high`, `auprc`, `brier_score`, `weighted_calibration_error`, `calibration_slope`, `calibration_intercept`. The CI bounds (2.5 / 97.5 percentiles of the per-bootstrap-sample AUC distribution, derived in the same bootstrap pass that draws the ROC band) ALSO appear in the KNN block — they are not exclusive to classical ML. The last two keys (`calibration_slope`, `calibration_intercept` — sharpness and bias diagnostics for probabilistic calibration) ARE exclusive to the classical-ML rows; the KNN rows carry `mean_ESS` instead, since neighbor-weight effective sample size has no classifier analogue. The file is opened in append mode (`'a'`), so the orchestrator MUST run KNN analysis (which writes `results.txt` with mode `'w'`) before classical ML, otherwise the appended ML block is overwritten on the next full pipeline run.
...
* **Semantic-Feature Ablation Study** *(scaffolding in progress, 2026-05-12)* — Separate module that perturbs a named clinical concept in the deterministic narrative, re-embeds the cohort, and re-fits `classical_ml.py` on `EMBEDDED` only to measure the ROC / AUPRC / Brier / calibration delta vs baseline. This is the only path to *per-concept* attribution on the embedded side, since the embedding classifier sees 4096 entangled latents rather than named columns. **Architecture: list-based registry, single orchestrator loop, env-var-bridged chokepoint.** `scripts/data_loading/ablation_registry.py` exports `ABLATIONS`, a Python list of override-spec dicts — either field-level (`{"id": "swap_race", "field": "Race_Ethnicity", "strategy": "permute"}`) or section-level (`{"id": "swap_psych_history", "section": "### PSYCH HISTORY", "strategy": "permute_section"}`). Every spec carries a filesystem-safe `id` because that string suffixes `NARRATIVES_DIR`, `EMBEDDINGS_DIR`, and `RESULTS_DIR` per iteration — each ablation gets a sibling directory tree so baseline artifacts and other ablations' artifacts are never overwritten. A single orchestrator script (TBD) iterates `ABLATIONS` and per spec: sets `ABLATION_ID` plus the three suffixed dirs in the subprocess environment, regenerates narratives, runs `forge_embeddings` (each ablation's `embeddings.db` is its own SQLite cache — re-runs reuse rows automatically via `PatientEmbedder`), and runs `classical_ml.py` on `EMBEDDED` only. `forge_embeddings` and `classical_ml.py` need **zero code changes** — they already read those env vars. The narrative chokepoint in `deterministic_narrative.py` reads `ABLATION_ID` once, looks up the matching spec in `ABLATIONS`, and dispatches on `spec["strategy"]` (NOT on `spec["id"]`) — adding the 6th, 50th, 500th ablation = one new dict entry in `ABLATIONS` with **zero chokepoint edits**; only inventing a brand-new strategy requires a dispatch branch. **Replacement strategy: cohort permutation, not neutralization.** For each anchor patient, the perturbed field's value is overwritten with the same field's value drawn from a randomly chosen *donor* patient in the cohort (self excluded). The embedder sees a real, in-distribution value; the field's correlation with the anchor's outcome is severed; the field's marginal distribution is preserved exactly because every value the embedder reads during a permuted run is a value it would read during a real patient's narrative. Earlier neutralization-with-placeholders (`"Missing"`, `"Absent"`, `"0"`) was rejected on mentor recommendation: writing `"Missing"` into a populated field is *false missing information*, planting a signal the embedder may have learned distinct semantics for and confounding the ablation delta with a missingness signal we did not intend to measure. Section-level permutation draws ONE donor per anchor and copies all section fields jointly to preserve intra-section clinical coherence. The donor RNG is seeded on `(anchor_id, ABLATION_ID, SEED)` for per-anchor per-ablation reproducibility. Length and positional encoding stay stable because section headers and field labels are preserved; only values move. The remaining confound — co-correlation between clinically related features — is irreducible and requires multi-field ablations for joint concepts. Run order per ablation: narrative regen → full cohort re-embed (the wall-clock bottleneck, on the c3_accel partition) → `classical_ml.py` on `EMBEDDED` only. KNN and LLM judging are out of scope. Output: `ablation_summary.csv` tabulating metric deltas vs baseline per classifier per ablation. **Locked slate (5 ablations, 2026-05-12):** `swap_race` (field — Race_Ethnicity), `swap_psych_history` (section — `### PSYCH HISTORY`), `swap_med_burden` (section — `### MEDICATION BURDEN`), `swap_safety` (section — `### SAFETY`), `swap_sdoh` (field — SDOH, special-cased: not a JSON demographics key but a single rendered line). `### TREATMENT EXPOSURE` intentionally excluded because prior adequate AD trials are part of how TRD is *defined* in the cohort — the delta would be enormous, predictable, and uninformative. `suicide_flag` not ablated standalone — already covered under `swap_psych_history`. Lives outside `feature_importance.py` because ablation is pipeline orchestration (re-run upstream stages, diff downstream metrics), not classifier introspection.
