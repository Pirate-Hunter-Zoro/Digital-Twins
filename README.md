...
* **Publication target** — When this work is written up, the manuscript targets *JMIR Mental Health* (JMIR Publications). Format the draft to JMIR's submission template before drafting prose so figure captions, methods structure, and reporting checklists (TRIPOD-AI for prediction-model reporting) align with the journal's requirements from the start rather than being retrofitted. The draft lives in `writeup/manuscript.md` (Methods → Results → Discussion; Markdown, `pandoc manuscript.md -o manuscript.docx` into the JMIR template at submission) with supporting material in `writeup/supplement.md`. The main text is deliberately curated to the headline metrics — ROC AUC, confusion matrices, calibration curves with slope/intercept, per-classifier FEATURE feature importance, the semantic-feature ablation, and the cross-embedder comparison — while secondary analyses are relegated to four supplements: **S1** (LLM clinical-similarity judge), **S2** (embedded-representation effective dimensionality: L1 sparsity, cumulative |Spearman ρ|, PCA-K sweep), **S3** (precision–recall / AUPRC), and **S4** (calibration absolute-error metrics: Brier, weighted calibration error). The manuscript review/curation pass has run through the Results "Density and chronology checks" section; "Provenance of performance results" onward is pending re-check against the relocated metrics.
