...
* **`feature_vector.py`**: Constructs a typed `pandas.Series` of features for each patient from the sliced JSON, with explicit dtypes that drive downstream preprocessing:
  * `float64` — quantitative features (vitals, counts, days, age, adequate-trial counts). The three cohort-averaged vital columns (`bmi`, `bp_sys`, `bp_dias`) sit at 63-69% missingness across the cohort with an 8-10 percentage-point TRD-stratified missingness delta — diagnostic and cross-tab in `notebooks/cohort_investigation.ipynb`. How the FEATURE classical-ML pipeline handles the resulting NaNs is parameterized by a three-arm `VitalsStrategy` sensitivity loop (DROP / INDICATOR / ASYMMETRIC); see Planned Extensions below. EMBEDDED-side narratives encode the missingness independently via the deterministic narrative's `"Missing"` sentinel, so the FEATURE strategy choice has no bearing on existing embeddings or embedded classifier fits.
...
* **`classical_ml.py`**:
  ...
  * **Classifiers** (4 total):
    ...
    * Random Forest
    * Histogram-based Gradient Boosting (`HistGradientBoostingClassifier`) — NaN-native; required for the `VitalsStrategy` sensitivity arms that feed NaN-bearing matrices to the tree branch (INDICATOR / ASYMMETRIC)
  ...
  * **Hyperparameter Tuning**: Every classifier is wrapped in `GridSearchCV(pipeline, param_grid, scoring='roc_auc', cv=5, n_jobs=-1)` before fitting; `predict_proba` delegates to the refit best estimator. Parameter grids are declared at module level in `HYPERPARAMETERS`.
    ...
    * Per-classifier `best_params_` and `best_score_` are persisted to `grid_search_ml_results_{source}.json` in `RESULTS_DIR` (one file per `VectorSource`; under FEATURE, also suffixed with `_{strategy.name}` per `VitalsStrategy` arm — see below).
  * **Dual-Source Evaluation with FEATURE strategy loop**: `main()` runs an EMBEDDED pass (single load, four classifiers, no strategy axis) followed by a FEATURE pass that iterates `for strategy in VitalsStrategy:`. Under DROP / INDICATOR the load is shared across all four classifiers; under ASYMMETRIC `main()` dual-loads the LR matrix (vital-stripped via `classifier_family=LINEAR`) and the tree matrix (vital-retained via `classifier_family=TREE`) and calls `evaluate_models` twice with the `only_classifier_family` filter, merging the two result dicts before downstream plotting. 12 FEATURE fits per run (4 classifiers × 3 strategies) plus 4 EMBEDDED fits = 16 total. All ROC, Precision-Recall, Calibration, Decision Curve Analysis, and Optimal Confusion Matrix plots carry source-prefixed filenames, with `_{strategy.name}` suffix added under FEATURE only — full parity with the KNN diagnostic suite except for the Effective Sample Size distribution (KNN-exclusive; no neighbor-weight analogue exists for a fitted classifier).
  * **Fitted-Model Caching**: After each `GridSearchCV.fit`, the entire fitted searcher is `joblib.dump`-ed to `RESULTS_DIR/trained_models/{model_name}_{source.name}.joblib`, or `..._{model_name}_{source.name}_{strategy.name}.joblib` under FEATURE so the three sensitivity arms coexist on disk. On subsequent runs, if the cache file exists and `SCRUB_TRAINED_MODELS != 1`, the searcher is loaded from disk and the grid search is skipped — `predict_proba`, `best_params_`, `best_score_`, and `cv_results_` all come back intact. This makes the analysis pipeline robust to mid-`analysis_only.sbatch` crashes: re-runs skip past the cached classifiers and resume at whatever blew up. `feature_importance.py::refit_best_model` consumes the same cache (returns `joblib.load(cache_path).best_estimator_`) and falls back to its original rebuild-and-refit path on cache miss for standalone runs; it forwards `vitals_strategy` and `classifier_family` so the cache lookup and fallback both target the right per-strategy artifact. The PCA-K sweep in `plot_pca_k_vs_roc` caches each `(model_name, K)` pipeline to a sibling directory `RESULTS_DIR/trained_models_pca/` under the same `SCRUB_TRAINED_MODELS` flag (EMBEDDED-only, no strategy axis).
  ...
  * **Data Loading**: `load_data_set()` accepts `(patient_ids, source, vitals_strategy=None, classifier_family=None)`. Feature mode loads the cohort parquet file at `FEATURE_DATAFRAME_PATH` and slices rows by train/test ID; `vitals_strategy=DROP` strips `VITAL_COLUMNS` at parquet-load time, and `vitals_strategy=ASYMMETRIC` with `classifier_family=LINEAR` also strips (matching DROP for the LR call-site only). INDICATOR, None, and ASYMMETRIC+TREE return the full vital-retained matrix; the per-classifier pipeline shape produced by `make_numeric_transformer` decides how NaN is handled downstream (LR-INDICATOR gets `SimpleImputer(median, add_indicator=True)` + `StandardScaler`; trees under INDICATOR / ASYMMETRIC use a `"passthrough"` numeric branch and rely on NaN-native splits). Embedded mode queries `embeddings.db` with a batched `SELECT ... WHERE patient_id IN (...)` query, ordered by patient ID to maintain alignment with labels, returning a DataFrame whose columns are all `float64`; `vitals_strategy` and `classifier_family` are ignored under EMBEDDED.
  * **Feature Importance**: Scoped to sibling module `feature_importance.py` covering both sources; runs after `classical_ml.py` inside `analysis_only.sbatch` and consumes the same cache. The module is `VitalsStrategy`-aware on the FEATURE side: an outer `for strategy in VitalsStrategy:` loop loads `matrices_by_family` (two loads under ASYMMETRIC, one shared load otherwise) and the inner classifier loop dispatches per-family via `CLASSIFIER_FAMILY[model_name]`, so each `(classifier, strategy)` cell trains-or-loads against the matrix shape its `classical_ml.py` counterpart was fitted on. Per-strategy outputs land in `RESULTS_DIR/feature_importance/`: 12 `feature_importance_{classifier}_{strategy}.png` bar charts and 3 `feature_importance_summary_{strategy}.json` files. EMBEDDED side is unchanged (single pass, cumulative correlation + cumulative importance overlays, PCA-K sweep, sparsity diagnostic). See the Planned Extensions section below.
...
* **Feature Importance** — Sibling module `scripts/digital_twins/predictions/feature_importance.py`, consumed by `classical_ml.py` (or runnable standalone against persisted fits). The module loops over both `VectorSource` values but answers a different question per source. The two paths share `load_best_params` / `refit_best_model` plumbing and a shared univariate-correlation helper, but produce different outputs because feature-level interpretability does not exist on the embedded side. **Status (2026-05-11): all primitives, `main()` orchestrator, and sbatch integration (both `run_trd_prediction_analysis.sbatch` and `analysis_only.sbatch`) landed. Module runs after `classical_ml.py` and reads `grid_search_ml_results_{source}.json`. The EMBEDDED LR sparsity diagnostic is emitted as `feature_importance_sparsity.json` (keys: `nonzero_coefficients`, `total_coefficients`) — the predictions package is fully JSON-only, no `results.txt` writers remain. Update (2026-05-18): the cumulative-overlay helper was generalized to `plot_cumulative_magnitude_overlay` (takes any per-dim magnitude array, paired with caller-supplied xlabel/ylabel/title/filename). The existing Spearman-correlation overlay was renamed on disk from `feature_importance_cumulative_EMBEDDED.png` to `feature_correlation_cumulative_EMBEDDED.png` to free that filename for the parallel built-in-importance overlay. Both overlays now emit: the EMBEDDED classifier loop populates an `importances_by_label` dict (LR via `coef_[0]`, tree models via `feature_importances_`), and a second `plot_cumulative_magnitude_overlay` call after the loop writes `feature_importance_cumulative_EMBEDDED.png` with importance-flavored axis labels and title. Update (2026-05-21): module is now `VitalsStrategy`-aware on the FEATURE side. `load_best_params`, `refit_best_model`, `plot_feature_importance`, and `write_feature_importance_summary` accept `Optional[VitalsStrategy]` and splice `_{strategy.name}` into their respective filenames; `refit_best_model` also takes `Optional[ClassifierFamily]` so the cache-miss fallback builds the right numeric branch via `make_classifier(model, strategy, family)`. `main()` is split into an EMBEDDED pass (unchanged) followed by a FEATURE pass with an outer `for strategy in VitalsStrategy:` loop; inside each strategy, `matrices_by_family` keyed by `ClassifierFamily` (two loads under ASYMMETRIC, one shared load otherwise) feeds per-classifier dispatch via `CLASSIFIER_FAMILY[model_name]`. Per-strategy outputs: 12 `feature_importance_{classifier}_{strategy}.png` and 3 `feature_importance_summary_{strategy}.json`. Stale `GradientBoostingClassifier` references in the refit fallback and `plot_pca_k_vs_roc` swapped to `HistGradientBoostingClassifier` for parity with `classical_ml.py`'s active estimator.**
