...
1.**Raw EHR Data Processing (New Initial Step):** Transform raw, tabular EHR data (CSV/RDS) into a standardized, patient-centric JSON format (e.g., `all_patients_combined.json`), suitable for LLM consumption. This involves robust data cleaning, SQLite ingestion, and parallel JSON generation.
2.**Define a Target:** Select a patient and their historical visit sequence up to visit `k` from the processed EHR data.
3.**Create a Candidate Pool:** From all other patients in the dataset, generate a comprehensive library of all possible historical visit sequences of length `k`.
