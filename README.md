...
* **`run_neighborhood_constructor.py`**:
  ...
  * Builds one `TRDPredictor` (test set as `exclude_ids`), flattens every (anchor, neighbor, scheme) pair into a work list synchronously (fast cosine retrieval + narrative lookups), then fires all LLM judgements through `asyncio.gather`, throttled by an `asyncio.Semaphore(LLM_MAX_CONCURRENCY)` so at most `LLM_MAX_CONCURRENCY` requests are in flight against the vLLM server at once.
