...
* **`feature_vector.py`**: Constructs a typed `pandas.Series` of features for each patient from the sliced JSON, with explicit dtypes that drive downstream preprocessing:
  * `float64` — quantitative features (vitals, counts, days, age, adequate-trial counts). The three cohort-averaged vital columns (`bmi`, `bp_sys`, `bp_dias`) sit at 63-69% missingness across the cohort with an 8-10 percentage-point TRD-stratified missingness delta. The cohort-level packaging diagnostic in `notebooks/cohort_investigation.ipynb` shows P(all 3 missing | ≥1 missing) = 90.3% — vitals are missing in packages, not independently. How the FEATURE classical-ML pipeline handles the resulting NaNs is parameterized by a three-arm `VitalsStrategy` sensitivity loop (DROP / INDICATOR_ONLY / NAN) — no imputation, since packaged missingness denies multivariate imputers their within-row signal and a median fill paired with a missingness indicator is mathematically absorbed by the indicator coefficient; see Planned Extensions below. EMBEDDED-side narratives encode the missingness independently via the deterministic narrative's `"Missing"` sentinel, so the FEATURE strategy choice has no bearing on existing embeddings or embedded classifier fits.
...
* **`classical_ml.py`**:
  ...
  * **Pipeline**: Each classifier is wrapped in a `sklearn.pipeline.Pipeline` with a dtype-routed `ColumnTransformer`. The numeric branch shape is strategy-dispatched by `make_numeric_transformer(vitals_strategy)`:
    * Numeric branch under DROP / INDICATOR_ONLY (and under EMBEDDED): `StandardScaler` only. The matrix has no NaN cells in these arms — DROP strips `VITAL_COLUMNS` entirely; INDICATOR_ONLY replaces the values with boolean indicator columns that flow through the bool branch; EMBEDDED is a dense `float64` block from the embedder.
    * Numeric branch under NAN (XGB only): `"passthrough"`. Preserves NaN cells for XGBoost's native NaN-aware splits; no imputation anywhere.
  ...
  * **Classifiers** (4 total):
    ...
    * XGBoost (`eval_metric='logloss'`) — NaN-native; the only classifier that runs the `VitalsStrategy.NAN` arm
  ...
  * **Dual-Source Evaluation with per-classifier FEATURE strategy loop**: `main()` runs an EMBEDDED pass (single load, four classifiers, no strategy axis) followed by a FEATURE pass that iterates a nested (classifier, strategy) loop, emitting only the legal cells per `LEGAL_STRATEGIES`. LR / RF / GB run DROP and INDICATOR_ONLY; XGB additionally runs NAN. 9 FEATURE fits per run plus 4 EMBEDDED fits = 13 total. All ROC, Precision-Recall, Calibration, Decision Curve Analysis, and Optimal Confusion Matrix plots carry source-prefixed filenames, with `_{strategy.name}` suffix added under FEATURE only — full parity with the KNN diagnostic suite except for the Effective Sample Size distribution (KNN-exclusive; no neighbor-weight analogue exists for a fitted classifier).
  * **Fitted-Model Caching**: After each `GridSearchCV.fit`, the entire fitted searcher is `joblib.dump`-ed to `RESULTS_DIR/trained_models/{model_name}_{source.name}.joblib`, or `..._{model_name}_{source.name}_{strategy.name}.joblib` under FEATURE so the per-classifier legal-arm cells coexist on disk. On subsequent runs, if the cache file exists and `SCRUB_TRAINED_MODELS != 1`, the searcher is loaded from disk and the grid search is skipped — `predict_proba`, `best_params_`, `best_score_`, and `cv_results_` all come back intact. This makes the analysis pipeline robust to mid-`analysis_only.sbatch` crashes: re-runs skip past the cached classifiers and resume at whatever blew up. `feature_importance.py::refit_best_model` consumes the same cache (returns `joblib.load(cache_path).best_estimator_`) and falls back to its original rebuild-and-refit path on cache miss for standalone runs; it forwards `vitals_strategy` so the cache lookup and fallback both target the right per-strategy artifact. The PCA-K sweep in `plot_pca_k_vs_roc` caches each `(model_name, K)` pipeline to a sibling directory `RESULTS_DIR/trained_models_pca/` under the same `SCRUB_TRAINED_MODELS` flag (EMBEDDED-only, no strategy axis).
  ...
  * **Data Loading**: `load_data_set()` accepts `(patient_ids, source, vitals_strategy=None)`. Feature mode loads the cohort parquet file at `FEATURE_DATAFRAME_PATH` and slices rows by train/test ID; `vitals_strategy=DROP` strips `VITAL_COLUMNS` at parquet-load time. `vitals_strategy=INDICATOR_ONLY` strips the vital VALUES AND appends three pre-computed boolean indicator columns (`missing_bmi`, `missing_bp_sys`, `missing_bp_dias`) derived from the original NaN positions before the strip — no fabricated numbers anywhere in the matrix. `vitals_strategy=NAN` returns the vital-retained matrix with NaN cells intact (XGB only — LR / RF / GB raise on NaN). Embedded mode queries `embeddings.db` with a batched `SELECT ... WHERE patient_id IN (...)` query, ordered by patient ID to maintain alignment with labels, returning a DataFrame whose columns are all `float64`; `vitals_strategy` is ignored under EMBEDDED.
  * **Feature Importance**: Scoped to sibling module `feature_importance.py` covering both sources; runs after `classical_ml.py` inside `analysis_only.sbatch` and consumes the same cache. The module is `VitalsStrategy`-aware on the FEATURE side: a nested (classifier, strategy) loop dispatches per `LEGAL_STRATEGIES`, so each legal cell trains-or-loads against the matrix shape its `classical_ml.py` counterpart was fitted on. Per-strategy outputs land in `RESULTS_DIR/feature_importance/`: 9 `feature_importance_{classifier}_{strategy}.png` bar charts and 3 `feature_importance_summary_{strategy}.json` files. EMBEDDED side is unchanged (single pass, cumulative correlation + cumulative importance overlays, PCA-K sweep, sparsity diagnostic). See the Planned Extensions section below.
...
* **FEATURE-side vitals-handling sensitivity analysis** *(refactor in progress; aggregator pending)* — Three-arm sensitivity comparison over how the FEATURE classical-ML pipeline handles the three cohort-averaged vital columns (`bmi`, `bp_sys`, `bp_dias`), which sit at 63-69% missingness across the cohort with an 8-10 percentage-point TRD-stratified missingness delta. The cohort packaging diagnostic in `notebooks/cohort_investigation.ipynb` shows P(all 3 missing | ≥1 missing) = 90.3% — vitals are missing in packages, not independently. Two implications: (1) multivariate within-row imputers (KNN, MICE) have no within-vital signal to chain across the three vitals and degenerate to imputing from non-vital regressors, which only reproduces the engagement-disparity signal already encoded by the missingness flag itself; (2) median imputation paired with a missingness indicator is mathematically absorbed by the indicator coefficient under LR and largely absorbed under tree splits when the tree branches on the indicator first, so filling a placeholder value into a flagged-missing cell adds no scientific signal while injecting cohort-median-flavored numbers into cells where the truth is "unknown". Conclusion: no imputation at all. **Arms.** *DROP* strips `VITAL_COLUMNS` at `load_data_set` time; all four classifiers see the same vital-free matrix. *INDICATOR_ONLY* strips the vital VALUES (same matrix shape as DROP for the value columns) and appends three pre-computed boolean indicator columns (`missing_bmi`, `missing_bp_sys`, `missing_bp_dias`) derived from the original NaN positions before the strip — no fabricated numbers anywhere. *NAN* is a passthrough with NaN cells intact; only XGBoost can run it (LR / RF / GB raise on NaN). **Per-classifier legal arms.** `LEGAL_STRATEGIES` in `scripts/shared/utils.py` maps each classifier to its tuple of permitted arms: LR / RF / GB get `(DROP, INDICATOR_ONLY)`; XGB gets `(DROP, INDICATOR_ONLY, NAN)`. 9 FEATURE fits per run plus 4 EMBEDDED fits = 13 total. `classical_ml.py::main()` runs the EMBEDDED pass then iterates the FEATURE branch as a nested (classifier, strategy) loop emitting only legal cells. No `ClassifierFamily` shim, no ASYMMETRIC dual-load, no imputer. Artifact filenames extend the existing `{model}_{source}` suffix to `{model}_{source}_{strategy}` when strategy is set; EMBEDDED artifacts retain the shorter suffix. EMBEDDED source is single-pass — `VitalsStrategy` does not apply (no NaN, no named vital columns; existing embeddings remain valid). Not an `.env` addition — `VitalsStrategy` is a pipeline-internal iteration axis. `feature_importance.py` mirrors the same legal-arm dispatch so each legal `(classifier, strategy)` cell trains-or-loads against the matrix shape its `classical_ml.py` counterpart was fitted on; per-strategy bar charts and summary JSONs land under `RESULTS_DIR/feature_importance/`. **Comparison surface (pending).** A sibling aggregator module (likely `scripts/digital_twins/predictions/vitals_sensitivity.py`, wired into `analysis_only.sbatch` after `feature_importance.py` and before `ablation_runner.py`) reads the three `classical_ml_results_FEATURE_*.json` files and emits `vitals_sensitivity.csv` to `RESULTS_DIR/` (9 rows × 10 columns: `strategy`, `classifier`, plus the eight metric keys) and six grouped-bar `vitals_delta_*.png` plots (one per metric: `roc_score`, `auprc`, `brier_score`, `weighted_calibration_error`, `calibration_slope`, `calibration_intercept`), mirroring the `ablation_delta_*` plot pattern; the `roc_score` PNG carries asymmetric CI whiskers, the other five carry bars only. Lives outside `classical_ml.py` because the aggregator runs once *across* the three strategy outputs, not per-strategy. **Latent caveat.** Under the XGB-NAN cell, `compute_univariate_spearman` returns NaN correlations for vital columns; `np.sign(NaN) >= 0` then fails and degrades XGB's direction-of-effect coloring on that one cell. LR / RF / GB under DROP and INDICATOR_ONLY are unaffected (no NaN ever reaches the correlation helper). Fix candidates: `spearmanr(..., nan_policy='omit')` plus skip-color on NaN-sign entries, or suppress direction coloring on the single XGB-NAN cell. Tracked in `TODO.txt`.
